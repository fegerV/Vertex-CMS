...
class FormService
{
    ...
    public function validate(Form $form, Request $request): Validator
    {
        ...
        foreach ($form->fields as $field) {
            ...
            if (!in_array($field->name, $visibleFields, true)) continue;
    ...
    private function calculateTotal(Form $form, array $data): float
    {
        ...
        foreach ($form->fields as $field) {
            if ($field->type !== "calculator") continue;
    ...
    private function buildValidationRules(FormField $field): string
    {
        ...
        switch ($field->type) {
            ...
            case "textarea":
            case "text":
                if (isset($field->options["minlength"])) $rules[] = "min:" . $field->options["minlength"];
                if (isset($field->options["maxlength"])) $rules[] = "max:" . $field->options["maxlength"];
                if (isset($field->options["pattern"])) $rules[] = "regex:" . str_replace(["/", "|"], "-", $field->options["pattern"]);
                break;
        }
